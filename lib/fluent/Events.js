...
*/
          ...
          do {
            var last = undefined
            await this.api.readStream({ space: this.space.name, domain: this.domain.name, stream: this.stream.id, after, until: this.until || 'caught-up', limit: this.limit, each: async event => {
              last = event.meta.id
              await notify(event, callback)
            }})
            .catch(err => console.log(err))
...
*/
          ...
          do {
            var last = undefined
            await this.api.readDomain({ space: this.space.name, domain: this.domain.name, after: this.after, until: this.until || 'caught-up', limit: this.limit, each: async event => {
