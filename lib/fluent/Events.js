  ...
  async each(callback) {
    return new Promise(async (resolve, reject) => {
      ...
      if (this.after !== 'past-events') {
        if (this.stream) {
          await this.api.readStream({ space: this.space.name, domain: this.domain.name, stream: this.stream.id, after: this.after, until: this.until, each: async event => {
            await notify(event, callback)
          }})
          .catch(err => console.log(err))
        }
        else {
          await this.api.readDomain({ space: this.space.name, domain: this.domain.name, after: this.after, until: this.until, each: async event => {
            await notify(event, callback)
          }})
          .catch(err => console.log(err))
        }
      ...
      this.api.messages.on({ event: event => {
        notify(event, callback, subscription)
      }})

      // Subscribe to real-time notifications for stream or domain.
      this.api.messages.subscribe({ channel })

      // Resolve promise with real-time subscription in order to give caller a chance to
