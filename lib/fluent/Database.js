...
class Database {
  ...
  async domain({ folder, tools }) {
    ...
    const visit_folder = async function(base, dirs, fn) {
      const folder = path.resolve(base, ...dirs)
      const files = fs.readdirSync(folder)
      for (let file of files) {
        if (fs.statSync(`${folder}/${file}`).isDirectory()) return await visit_folder(base, [ ...dirs, file ], fn)
        if (file.toLowerCase().endsWith('.js')) await fn(base, dirs, folder, file, path.resolve(folder, file))
      }
    }

    // Register all domain objects.
    await visit_folder(path.resolve(folder, 'objects'), [], async (base, dirs, folder, file, filepath) =>
      this.objects.register(path.parse(file).name, require(filepath), toolbox)
    )

    // Register all domain views.
    await visit_folder(path.resolve(folder, 'views'), [], async (base, dirs, folder, file, filepath) =>
      this.views.register(`${[ ...dirs, path.parse(file).name ].join('.')}`, require(filepath), toolbox)
    )

    // Register all domain reactions.
    await visit_folder(path.resolve(folder, 'reactions'), [], async (base, dirs, folder, file, filepath) =>
      this.reactions.register(`${[ ...dirs, path.parse(file).name ].join('.')}`, require(filepath), toolbox)
    )
  }
