...
*/
  ...
  async domain({ folder, tools, objects, views, reactions }) {
    ...
    const docs = (collection) => ({
      write: async (doc) => await this.store.documents.write({ db: this.name, collection, doc }),
      read: async ({ id }) => await this.store.documents.read({ db: this.name, collection, id }),
      count: async (query) => await this.store.documents.count({ db: this.name, collection, query }),
      find: async (query, options) => await this.store.documents.find({ db: this.name, collection, query, options }),
      destroy: async ({ id }) => await this.store.documents.destroy({ db: this.name, collection, id })
    })

    // Configure toolbox with default and user-supplied tools.
    const toolbox = { ...tools, docs, db: this }
