...
class Instance {
    ...
    this.xstate = Machine(
      {
        ...
        states: transpileStates(type.workflow.states),
        initial: type.workflow.initial,
        strict: false
      },
      {
        actions: type.actions,
        services: transpileInvocations(type.invocations || {}, this.registry, this.write)
      }
    )
  ...
  data() {
    return { state: this.service.state.value, ...this.service.state.context }
  }
...
function transpileStates(states) {
...
function transpileInvocations(invocations, registry, write) {
