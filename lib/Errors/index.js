...
const request_errors = require('request-promise-native/errors')
...
Errors.handle = async function (callback) {
  try {
    ...
    if (error instanceof request_errors.StatusCodeError) {
      let message = ''
      try { message = JSON.parse(error.error).error }
      catch (error) { message = error.error }
      throw new Error(HttpStatus.getStatusText(error.statusCode) + ': ' + message)
    }
    throw error
  }
}
