...
module.exports = async function(store, { db, ns, from, after, to, until, types, filters, groupby, rollupto, limit, each }) {
  let events = store.events.filter(event => event.meta.db === db && event.meta.ns === ns)
