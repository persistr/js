  ...
  async fetch({ method, endpoint, query, body, headers }) {
    ...
    if (authorization) {
      ...
      if (this.identity.credentials.apikey) {
        ...
        //const encoded = Buffer.from(`${this.identity.credentials.email}:${this.identity.credentials.password}`).toString('base64')
    ...
    try {
      response = await fetch(`${this.server}${endpoint}${queryString(query)}`, options)
    }
    catch (error) {
    ...
    if (!response.ok) {
      throw new Errors.StatusCode(response.status, response.statusText, await response.text())
    }
