...
async function params(http, { method, server, endpoint, query, body, headers }) {
  ...
  if (http.identity && http.identity.authorization) {
    authorization = await http.identity.authorization()
  }
  if (authorization) {
    headers = { authorization, ...headers }
  }
  else if (http.identity && http.identity.credentials) {
    if (http.identity.credentials.apikey) {
      headers = { authorization: `Apikey ${http.identity.credentials.apikey}`, ...headers }
    }
    else if (http.identity.credentials.email && http.identity.credentials.password) {
      const encoded = Buffer.from(`${http.identity.credentials.email}:${http.identity.credentials.password}`).toString('base64')
  ...
  if (body && result.headers['content-type'].includes('json')) result.body = JSON.stringify(body)
  return result
}
